...
pub fn testDjotToHtml(djot_source: [:0]const u8, expected_html: []const u8) !void {
    ...
    var html = std.ArrayList(u8).init(std.testing.allocator);
    defer html.deinit();
    try djot.toHtml(std.testing.allocator, djot_source, html.writer());
    try std.testing.expectEqualStrings(expected_html, html.items);
}
